...
export default function PaymentDueAlerts({ days = 7 }: { days?: number }) {
  ...
  const dueItems = useMemo(() => {
    if (!cards || !expenses || !payments) return [] as any[];
    ...
    return cards
      ...
      .filter(
        (it) =>
          it.metrics.amountToPayNext > 0 &&
          it.daysUntil >= 0 &&
          it.daysUntil <= days,
      );
  }, [cards, expenses, payments, today, days]);
              ...
              <button
                onClick={async () => {
                  const ok = await showConfirm(
                    `Mark ₹${metrics.amountToPayNext} as paid for ${card.title}?`,
